...
def main():
    ...
    gt = phantom
    mask = make_var_dens_mask(N, N, accel=accel, center_fraction=center_fraction, seed=123)
    ...
    x_rec, logs = IHT(
        y, A, AT,
        k=k,                 
        T_fwd=T_fwd, T_inv=T_inv,
        step=step, iters=iters,
        proj=proj,
        x_init=AT(y),        
        eps_mode="x", eps_abs=eps_abs, eps_rel=0.0, min_iters=min_iters,
        x_ref=gt
    )
